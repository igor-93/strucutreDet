...
import time
import numpy as np
from mcl_impl import r_mcl_implementation, draw
import scipy.io
...
def r_mcl(instance_path, res_folder, strategy=2):
	...
	total_time = 0
	for inf_iter in range(8):
		gc.collect()
		labels = [-1 for x in range(n)]

		print '#############################################################'
		inflate_factor = 1.1 + inf_iter/10
		#inflate_factor = 1.1 + inf_iter/10
		print 'DEBUG: iteration = ', iteration
		print 'DEBUG: inflate = ', inflate_factor
		try:
			labels = np.load(res_folder+file+'_rmcl_'+str(inf_iter)+'_sim'+str(strategy)+'.npy')
			print 'Existing clustering found for these parameters.' 
			print res_folder+file+'_rmcl_'+str(inf_iter)+'_sim'+str(strategy)+'.npy'

		except:
			print 'NO existing clustering found for these parameters. Running R_MCL....'
			...
			clusters = r_mcl_implementation(sim_matrix, 
								inflate_factor = inflate_factor, max_loop = 20 , mult_factor = 1)
			...
			np.save(res_folder+file+'_rmcl_'+str(inf_iter)+'_sim'+str(strategy), labels)

		############################################
		# good visualization
		#draw(sim_matrix, cluster_map=clusters)
		############################################

		# continue if all points in 1 cluster
		if len(set(labels)) == 1:
			print 'skiping this iteration...'
			nr_cl_with_diff_params += [1]
			continue

		# break if all points in different clusters
		if len(set(labels)) >= 0.8*n:
			print 'stopping...'
			nr_cl_with_diff_params += [len(set(labels))]
			break

		n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
		

		nr_cl_with_diff_params += [n_clusters]
		...
		if non_clustered == 0:
			...
			break;
		''' THIS IS INCORRECT
		if (iteration <> best_iteration or iteration <> sec_best_iteration)	and\
			best_iteration <> -1 and sec_best_iteration <> -1:
			print 'it does not make any sense to run any further, stopping...'
			break'''
		iteration += 1
		# ----------------------------------------------------------------------------------
		print '#############################################################'
	...
	if best_iteration >= 0:
		...
		best_dec = file + '_rmcl_' + str(best_n_clusters) + '_' + str(best_non_clusterd) +'_sim'+str(strategy)
	...
	if sec_best_iteration >= 0:
		...
		s_dec = file + '_rmclSTD_' + str(s_best_n_clusters) + '_' + str(s_best_non_clusterd) +'_sim'+str(strategy)
